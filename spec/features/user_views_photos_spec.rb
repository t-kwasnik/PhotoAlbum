...
# I must be logged in to see my photos
    ...
    scenario 'user can add selection to existing map', js: true do
        sign_in_as(user)

        all(".leaflet-marker-icon")[0].click
        all(".leaflet-marker-icon")[1].click
        select "My First Map", from: "Add To Map"
        click_on "Submit"
        ...
        expect( match_url(page.find("#photo#{photo2.id}")['src'], photos2.image_url )).to eq(true)
    end
end
